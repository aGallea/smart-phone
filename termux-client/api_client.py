...
class BackendClient:
    ...
    async def speech_to_text(self, audio_data: bytes) -> Optional[str]:
        ...
        try:
            ...
            async with self.session.post(f"{self.backend_url}/api/stt", data=data) as response:
                if response.status == 200:
    ...
    async def generate_response(self, user_input: str, context: Dict[str, Any] = None) -> Optional[str]:
        """
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(f"Generate response failed with status {response.status}")
                    return None
